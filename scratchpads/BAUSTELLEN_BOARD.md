...
| O5 | Per-Instrument EchoelSynth (#23) | ältere Serie | **PLAN+Council ✓** (`PLAN_PER_LANE_PATCH.md`): Kern da (lane.patch + slotPatchSink), Lücke = Editor-Tür editiert globalen Synth statt lane.patch. Slices: S1 setLanePatch+Tests [pur/Linux-CI] → S2 .patch(lane)-Payload → S2b Primär-Sink → S3 Live-Apply [gerät-gated]. Nächste: **S1 bauen** | Patch-Wechsel pro Spur hörbar (2 Spuren, 2 Klänge) |
