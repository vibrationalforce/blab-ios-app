...
- **Root view:** `Studio/WorkspaceView.swift` (`@main` → `WindowGroup` → `WorkspaceView`) — the workspace shell that hosts surfaces (e.g. `.compose`). `Studio/EchoelStudioView.swift` is the **Compose surface** (Compose · FX · Mix · Piano-Roll · Well) above the live `BioStripView` (HR·HRV·Br·Coh with **tap-to-learn**, MIDI/OSC/event dots, play toggle), NOT the app root. The old `StudioRoot` Tools/Works/Sync/Well TabView is deprecated/removed.
- **Presentation (stability, 10.76.15):** `EchoelStudioView` drives **ONE** `.sheet(item:)` + **ONE** `.fullScreenCover(item:)` via computed bindings (`activeSheet`/`activeCover` derive the highest-priority modal from the existing `@State` flags; dismiss calls `clearAllSheets()`/`clearAllCovers()`). NEVER re-add multiple concurrent `.sheet`/`.fullScreenCover` modifiers to one view — two-true-at-once installs an invisible tap-blocking layer (the "can't click anything" hang) and risks the metadata launch-overflow.
...
- **Absent (not wired — do not claim as shipping):** RTMP/streaming, video capture/edit, multitrack audio. **Art-Net + sACN (unicast) are live.** Clips/Arrangement domain + tests are built but the **UI is not wired**. **VocoderCore / BioModulation** = pure tested cores, **not yet wired** (foundations). **FeedbackGuard** (audio-input live monitoring) and **BioVisualParams** (immersive flash-safe pulse) are now **wired**. See `docs/dev/FEATURE_MATRIX.md`.
