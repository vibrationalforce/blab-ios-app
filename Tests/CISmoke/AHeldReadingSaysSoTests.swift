...
// removed it.
//
// ⭐ IT MARKS RATHER THAN BLANKS, and that is the load-bearing design decision. The four sound
// producers poll `latestBio` and dedupe on `timestamp`, so a frozen source does NOT zero the
// timbre — it PARKS it at the last body and leaves it there. Blanking the number would claim the
...
// the COMPOSER uses to decide whether a take has a body at all. So a held row is precisely the
// state that prints `body=0` in the generate breadcrumb (#500), and retuning a source's window
// moves both together instead of leaving a screen behind. `testTheThresholdIsTheSourcesOwnWindow`
...
//     bracket passes there and the failures are findings about the row, not about a missing file.
...
// and although `no longer arriving` does appear twice raw (once in code, once in the footer's own
// rationale), a positive scan passes either way. It stays because #453 created ONE definition of
...
final class AHeldReadingSaysSoTests: XCTestCase {
    ...
    /// The mount AND the clock in one assertion, deliberately. A `TimelineView` that took its time
    /// from the context date would measure age on a different clock than `usableBio()` — the exact
    /// two-clock mismatch #434 paid for one file over.
    func testTheRowTicksAndReadsTheWallClock() throws {
        let src = try code(Self.fxView)
        XCTAssertTrue(src.contains("private struct AlwaysOnBioRow"),
                      "anchor missing — the always-on row is gone; fix the anchor, not the assertion")
        ...
        XCTAssertTrue(src.contains("channel.reading(in: frame, now: CFAbsoluteTimeGetCurrent())"),
                      "age must be measured on the wall clock — the same clock `usableBio()` uses — "
                      + "never on the TimelineView context date (#434)")
    }

    /// Paired with the mount on purpose (#343): a scan that only proved the tick exists would stay
    /// green on a tree that kept the ticker and dropped every trace of the held state.
