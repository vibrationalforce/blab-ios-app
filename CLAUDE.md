...
- **P1 "Sound complete" — ALREADY BUILT (audited 2026-07-01; corrects the old "Clips/Arrangement UI not wired" note):** the melodic/DAW core is done and wired — **polyphonic synth** (`PolySynthVoice`) + **bass** (`SubBassVoice`) + **hybrid sample/synth drums** (`BeatPlayer` + `DrumSynthVoice`); **full patch editor + presets** (`SynthPatch`/`PatchStore`/`PatchEditorView`, favorites/community/save-as, live-apply, tested — but **DOORLESS since 2026-07-25**, see below); **breakbeat loop-cut** (`LoopCutter`/`LoopBarLength` in the Studio UI); **MIDI export** (`exportMIDI()` + `ShareSheet`); Clips + Arrangement UI **DELETED** by the pure-instrument epic (#121 Slice 4 — `ClipView` 807dc0d, `ArrangeTimelineView` eb58e7a; `ClipStore`/`ArrangementStore`/`AutomationLane` model retires in Slice 5).
  **✅ CRAFT-TOOL DOORS — #131a SHIPPED (`f2cbf34`/`bda8f41`, 2026-07-25; supersedes the same-day "DOORLESS KEEPERS" note, which was itself half wrong):** the ONE craft-editor slot exists — `private enum CraftEditor` + `@State craftEditor` + a single `.sheet(item: $craftEditor)` whose content switch lives OUT of the body in `craftEditorSheet(_:)`. **Body presentation-modifier count = 12** (was 11; the 10.76.34 SIGSEGV chain was ~19). **The next editor is an enum case + a switch arm — NEVER another modifier.** A case is added ONLY together with its door (an undoored case is the lying-`toolItems` trap).
  ...
  · **Correction to a claim I made about the roll:** presenting it is NOT what publishes `MusicalFrame`. That publish is in `PianoRollModel` (`PianoRollView.swift:971`) on the shared sequencer tick, installed once at app start (`EchoelmusicApp.swift:732`) — so the visual/light output stage is lit whether or not the roll is open. The door is load-bearing for EDITING, not for the spine.
  · **NEEDS-FOUNDER-VERIFY:** launch (the +1 modifier vs the black-screen law) and the roll's Stop, which cascades the ONE-Stop law (`PianoRollView.swift:1206`) and ends the whole bio session rather than only playback.
...
loop size), generate + play, FX character. **Corrected 2026-07-25:** the "deeper
editors reachable via their remaining sheet slots (Piano Roll / patch editor / …)"
line was stale — the Piano-Roll and PatchEditor sheets were removed in v10.79.207,
so those two are DOORLESS today (task #131). Sample browser + audio clip doors still
stand. AUv3 hosting is gone (#121 Slice 2). The old Section/Tools table stood here
until 2026-07-04; do not resurrect it as fact.
