...
// So the wrong-format path had three outcomes and only one of them was loud.
//
// ⚠️ HONEST GRADING (§3). **11 assertion SITES** (Python, lines whose first token is
...
// driven over both trees): **6 RED on the parent, 0 on the worktree.** Red were claim 1, both
...
final class TheInputTapAsksTheNodeForItsFormatTests: XCTestCase {
    ...
    func testNoInstallTapIsHandedTheConnectTimeFormat() throws {
        ...
        for tap in afterEachTap {
            let args = String(tap.prefix(200))
    ...
    func testTheTapReadsTheNodeFormatItself() throws {
        let code = try source()
        XCTAssertTrue(code.contains("let tapFmt = input.inputFormat(forBus: 0)"), """
            Nothing re-reads `input.inputFormat(forBus: 0)` at the tap site. The format captured \
            at the top of the ON path has since survived a connect, an engine restart and a \
