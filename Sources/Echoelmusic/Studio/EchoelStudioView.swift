...
//   surviving two lines above the paragraph that rejects it.)
    ...
    private var moodPanel: some View {
        panel("Mood", "Character of the composition", isExpanded: $showMood) {
            ...
            Text("Friendly ↔ scary (tension) · sparse ↔ busy (liveliness) · odd leaps (weird). Blends with your live signal. Darkness and Romance switch rather than fade: above 0.60 Darkness drops the voicing an octave, and above 0.50 Romance adds the 7th to genres whose chord does not already have one (8 of the 18 offered).")
                .font(EchoelTheme.font(11)).foregroundStyle(EchoelTheme.dim)
                .fixedSize(horizontal: false, vertical: true)
        }
    }
    ...
    /// any setting. That is 10 of the 18 offered genres — `.selfObservation`, THE SHIPPED DEFAULT,
    /// among them. The caption therefore says "to genres whose chord does not already have one
    /// (8 of the 18 offered)" rather than promising the 7th outright; an unqualified promise would
    ...
    private func generate(startTransport: Bool = true,
        ...
        //  · "Pure Flächen run straight" is already what the DATA says: of the 18 genres in
        //    `MusicStyle.offered`, ELEVEN return `swing == 0` — the calm ones plus acid,
        //    trance and dark minimal. The hardwire did not protect them; it silenced the other seven.
        //
        ...
        // infer it. Both decisions it shifts sit behind `!sustained`, and 8 of the 18 offered
