...
| 4 | Secondary/unit text fails AA contrast (`dim` #e0e0e0@0.55 ≈4.4:1 on panel fills) | EchoelTheme.swift:20 | S | no | TODO — raise dim opacity ~0.55→0.65 (≈5.3:1). App-wide one-liner. VERIFY: does 0.65 stay Uncodixfy-muted, not too bright? |
| 5 | Bio strip can't grow w/ Dynamic Type (hard `.system(size:)` + minScale 0.6 → shrinks to ~7pt) | BioStripView.swift:141 | M | no (sim) | TODO — EchoelTheme.font (relativeTo) + reflow + cap minScale 0.85 |
