...
final class TimelineStoreAutomationEditTests: XCTestCase {
    ...
    @MainActor
    func testSetAutomationValue_revaluesTheKeyframeWithoutMoving() {
        ...
        XCTAssertEqual(lane?.points.first?.value, 0.9, accuracy: 1e-9)
    ...
    @MainActor
    func testUnknownParameterOrID_noCrashNoChange() {
        let store = TimelineStore()
        ...
        XCTAssertEqual(store.automationLane(forParameter: param)?.points.first?.value,
                       0.4, accuracy: 1e-9)
    }

    @MainActor
    func testAliasIdentity_precisionEditHitsTheSameLaneAsTheRow() {
        ...
        XCTAssertEqual(store.automationLane(forParameter: "masterLevel")?.points.first?.value,
                       0.25, accuracy: 1e-9, "alias edit must land on the same lane")
    }
}
