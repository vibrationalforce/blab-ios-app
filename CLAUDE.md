...
- **P1 "Sound complete" — ALREADY BUILT (audited 2026-07-01; corrects the old "Clips/Arrangement UI not wired" note):** the melodic/DAW core is done and wired — **polyphonic synth** (`PolySynthVoice`) + **bass** (`SubBassVoice`) + **hybrid sample/synth drums** (`BeatPlayer` + `DrumSynthVoice`); **full patch editor + presets** (`SynthPatch`/`PatchStore`/`PatchEditorView`, favorites/community/save-as, live-apply, tested — but **DOORLESS since 2026-07-25**, see below); **breakbeat loop-cut** (`LoopCutter`/`LoopBarLength` in the Studio UI); **MIDI export** (`exportMIDI()` + `ShareSheet`); Clips + Arrangement UI **DELETED** by the pure-instrument epic (#121 Slice 4 — `ClipView` 807dc0d, `ArrangeTimelineView` eb58e7a; `ClipStore`/`ArrangementStore`/`AutomationLane` model retires in Slice 5).
  **⚠ DOORLESS KEEPERS (task #131, corrected 2026-07-25 — the old "reachable from `EchoelStudioView`" claim was stale):** `PatchEditorView` AND `PianoRollView` have NO door in the shipping app. `EchoelStudioView`'s own sheets for both were removed in v10.79.207 (`EchoelStudioView.swift:361`), leaving `ArrangeTimelineView` as their sole construction site — and the timeline was unmounted 2026-07-24, then deleted in 4b. So the instrument today cannot edit a generated melody or shape/save a patch. Same for `ImmersiveStageView` (spatial stage). Re-dooring must NOT grow the sheet chain (black-screen law). Music theory is fully in-house. The real remaining frontier is **P3 Video** (no recorder/trim/export yet) and **P4 Broadcast**. See `scratchpads/PLAN_PRODUCT_STRUCTURE_2026-07-01.md`.
