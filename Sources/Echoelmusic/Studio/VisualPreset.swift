...
// bis Zentrifuge"). A preset is a PURE value type that sets the four live visual
// controls the user already tweaks with sliders — Intensity, Detail (ring density),
// Motion (flash-safe speed) and Spread — plus the render mode (spectrum donuts vs
// the bio rings). Selecting one is a launch point; the sliders then stay live for
// hands-on play. Every preset's values sit inside the existing clamped, WCAG
// flash-safe ranges, so a preset can never exceed the 3 Hz safety ceiling.
//
// Brand: the COLOUR is always the physically-correct tone→light mapping (Echoel CI
// is carried by EchoelTheme + SpectralColor), so presets shape FORM/ENERGY, not hue
// — keeping the science-first "the colour is the heard tone" promise intact.
...
public struct VisualPreset: Identifiable, Sendable, Equatable {
    ...
    /// true = spectrum→visible donuts; false = GPU bio rings (MetalBioView).
    public let spectralDonuts: Bool
    /// One-line character note for the picker.
    public let blurb: String

    public init(id: String, name: String, intensity: Float, detail: Float,
                motion: Float, spread: Float, spectralDonuts: Bool, blurb: String) {
        self.id = id
        self.name = name
        ...
        self.spectralDonuts = spectralDonuts
        self.blurb = blurb
    }
    ...
    public static let factory: [VisualPreset] = [
        VisualPreset(id: "aura", name: "Aura", intensity: 0.8, detail: 14, motion: 0.45,
                     spread: 1.35, spectralDonuts: true, blurb: "soft, sparse, slow halo"),
        VisualPreset(id: "drift", name: "Drift", intensity: 0.75, detail: 20, motion: 0.4,
                     spread: 1.4, spectralDonuts: false, blurb: "calm bio rings, wide and gentle"),
        VisualPreset(id: "bloom", name: "Bloom", intensity: 1.1, detail: 28, motion: 0.7,
                     spread: 1.2, spectralDonuts: true, blurb: "blossoming mid-density"),
        VisualPreset(id: "halo", name: "Halo", intensity: 1.3, detail: 24, motion: 0.6,
                     spread: 0.85, spectralDonuts: false, blurb: "bright, tight, focused ring"),
        VisualPreset(id: "cinder", name: "Cinder", intensity: 1.2, detail: 50, motion: 0.9,
                     spread: 0.95, spectralDonuts: true, blurb: "warm, dense, close embers"),
        VisualPreset(id: "pulse", name: "Pulse", intensity: 1.2, detail: 32, motion: 1.1,
                     spread: 1.0, spectralDonuts: false, blurb: "heartbeat-forward rings"),
        VisualPreset(id: "nebula", name: "Nebula", intensity: 1.0, detail: 72, motion: 0.6,
                     spread: 1.5, spectralDonuts: true, blurb: "vast, finely detailed field"),
        VisualPreset(id: "vortex", name: "Vortex", intensity: 1.15, detail: 60, motion: 1.3,
                     spread: 1.1, spectralDonuts: true, blurb: "fast, swirling, energetic"),
        VisualPreset(id: "zentrifuge", name: "Zentrifuge", intensity: 1.4, detail: 86, motion: 1.4,
                     spread: 1.2, spectralDonuts: true, blurb: "maximal — dense, fast, centrifugal")
    ]
}
