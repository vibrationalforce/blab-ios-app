...
| Stufe | Fähigkeit | Beleg |
|---|---|---|
| ⚠️ | MIDI **IN** (1.0/2.0 Noten + RTP-Netzwerk) · MIDI-**OUT** als virtuelle Quelle in jede DAW, inkl. **MPE out** (#713). ⛔ **Das ✅ und das „MPE **IN**" waren falsch** — #548/#770: keine Zonen-Unterscheidung (RPN 6,6 ohne Erzeuger), `event.channel` wird nirgends gelesen. Die zweite Begründung, die hier stand („gar kein Channel-Pressure-Zweig … Slide/Air/Pressure laufen in ein `break`"), ist seit #939/#942 abgelaufen: Press und Slide klingen, `.airCC` ist keine MPE-Dimension. Der Befund bleibt — **eine Zone ist das, was MPE hinzufügt**. | `Audio/MIDIInput`, `Audio/MIDIOutput` |
