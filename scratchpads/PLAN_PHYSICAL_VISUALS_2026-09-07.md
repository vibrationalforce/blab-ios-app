...
## Nachtrag 2026-09-07 (#1100) — der Founder hat die Wasserschale als zweites Register WIEDER GEÖFFNET

Wörtlich, Stunden nach diesem Plan: *„Moment, ich will auch wirklich reale Wasser Klang Bilder haben, wie als wenn ein Lautsprecher mit Wasser füllt."* Damit ist der Eintrag „THE FARADAY DISH … killed as anything near-term" oben **aufgehoben — als Scope-Entscheidung, nicht als Messung**: alles, was er misst (neue Feldfunktion + Bibliotheks-Zeile + Budget-Zeile + Pins + Docs), bleibt wahr und ist der Preis; der Founder hat ihn ausdrücklich bezahlt. Frage 2 dieses Plans (Register) ist damit beantwortet, Frage 1 (D10-Ausschluss neuer Looks) durch die Bitte selbst aufgehoben — für GENAU EINEN Look.

**Was unverändert bleibt, weil es Physik ist und keine Scope-Frage:** der NICHT-bauen-Eintrag „a chord-driven Faraday dish". Die Schale wird von EINEM Ton getrieben (`toneHz`). ⛔ Meine Antwort an den Founder versprach „aus bis zu fünf klingenden Noten gespeist" — zurückgenommen im Dateikopf von `Core/FaradayDish.swift`; die anderen Noten dürfen als FARBE ins Bild, nie als Gitter-Geometrie.

**Scheiben, neu geschnitten:**
- **Scheibe W1 = #1100 (gebaut):** `Core/FaradayDish` — Subharmonische, volle Dispersion (Bisektion, beide Zweige), viskose Schwelle a_c = 8νkω/tanh(kh), √-Onset, Kapillar-Anteil als Symmetrie-Treiber, „bis wohin es noch mustert". Wächter `TheWaterDishObeysFaradayTests` (END-TO-END, sechs Mutanten rot). Null Pixel. Die ZWEI gewählten Zahlen sind benannt: `speakerAccelerationAtFullDrive` = 200 m/s² (≈ 20 g, hergeleitet aus 100 dB SPL / 12-Zoll-Membran; Muster bis ≈ 1,3 kHz) und `saturationExcess` = 1.
- **Scheibe W2 = #1101 (gebaut):** Shader `fieldDish` — stehendes Gitter aus dem Wellenzahl-Uniform, Symmetrie aus `capillaryFraction` (Binks & van de Water: Quadrate → Sechsecke mit steigender Frequenz; die ABBILDUNG ist eine benannte Wahl), Beleuchtung = Kaustiken auf dem Schalenboden + Glanzlichter (Lampe von oben), Stärke = `patternStrength` (Stille = Spiegel). Kosten: Style-Slot — alle zehn belegt, Shader klammert 0…9; ein stillgelegter Index wird ERSETZT, nicht ein elfter angehängt. ⛔ Hier stand „1 Cymatics liegt nahe“ — falsch: Slot 1 ist die PLATTE, die Scheibe 2 dieses Plans exakt machen will. #1101 hat **Slot 2 (Plasma)** genommen: kein Tonbezug, keine Pins, kein Sonderfall im Farbpfad (Prism hätte einen). Blitz-Budget: die Stärke ist eine langsame Größe (Atem/Master ~1 Hz, geglättet), Phase nur im Wellen-Term — Herleitung MIT der Scheibe, nicht vorher geraten.
- **Scheibe W3 = #1102 (gebaut):** `LookBlendMap.library`-Zeile „Dish" (Index 2, NICHT in `defaultSequence` — wie Rings per Chip zuschaltbar) + FlashGuardTests-Budget-Zeile (0,40, kein Fold → 1,00 Hz) + Set-Pin, EIN Commit; die blockierende Hälfte pinnt beides aus `TheWaterDishIsLitLikeTheExperimentTests` Anspruch 7. Der Slew ist die Stärke-Glättung (tau 0,5 s) aus W2. ⛔ Die Website sagt weiter „four generative looks" — bewusst NICHT nachgezogen, bis der Founder das Bild gesehen hat (kein Anspruch vor dem Auge). Danach Founder-Auge am Gerät (NEEDS-FOUNDER-VERIFY: sieht es aus wie sein Foto; verblasst das Gitter bei der Tonhöhe, wo sein Ohr es erwartet — die Konstante dafür ist benannt).
