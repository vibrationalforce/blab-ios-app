...
· ~~[HIGH] `ci.yml` non-existent schemes~~ — FALSE POSITIVE (verified 2026-07-24: the
macOS/watchOS/tvOS/visionOS jobs run green via XcodeGen-generated schemes; `build-archive`
is branch-gated, not blocked) · [MED]
...
**B1 ~~[HIGH] `ci.yml` phantom schemes~~ — VERIFIED FALSE POSITIVE (2026-07-24, empirical
job-list of run 30068887406).** The `Build & Test (macOS)`, `Build (watchOS)`, `Build
(tvOS)`, `Build (visionOS)` jobs actually RUN and SUCCEED — the `Echoelmusic-macOS` etc.
schemes are generated by XcodeGen (the map worker's grep of `project.yml` missed them) and
build green; `Build Archive (Release)` is correctly branch-gated (`if: main`, skipped on
feature branches). NO fix — pruning them would have destroyed real multi-platform coverage.
The only *cost* note (not a bug): Echoel is iOS-only for v1.0, so building 4 deferred
platforms every push is CI minutes for deferred targets — a founder-gated CI-config
optimization, NOT a correctness item. **All of Tier B touches CI config → founder-gated per
CLAUDE.md DO-NOT ("Modify CI config without asking"); B2 (#86) also needs a portal
capability grant only the founder can do. Verify-only autonomously; do not edit `ci.yml`/
`testflight.yml` without a founder ask.**
