...
| O5 | Per-Instrument EchoelSynth (#23) | ältere Serie | **PLAN+Council ✓**. **S1 ✓ (b2f7750):** `setLanePatch` + 4 Tests. **S2 ✓ SHIPPED (3280aa9+aef9b67):** `.patch(lane)`-Payload — Editor seedet aus + persistiert `lane.patch` (onApply captured latest, onDismiss persistiert einmal, nur bei Änderung; dismiss-race-gehärtet); code+ui-state-reviewer CLEAN. Slices offen: **S2b Primär-Sink** (nächste — `lane.patch` der Primär-Lane bei Play/Region-Load auf globale Stimme anwenden; reviewer+audio-thread) → S3 Live-Apply Sekundär-Lane [gerät-gated]. | Patch-Wechsel pro Spur hörbar (2 Spuren, 2 Klänge) — Gerät-Hörtest |
