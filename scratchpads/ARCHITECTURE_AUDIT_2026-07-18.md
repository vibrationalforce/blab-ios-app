...
4. **✅ ERLEDIGT (0077a59, audio-thread-reviewer APPROVED):** `guard !isRecording` in `MicrophoneManager.startRecording` + `stopRecording` deaktiviert nicht mehr die prozessweite Session (`setActive(false)` → `AudioConfiguration.downgradeToPlaybackAfterRecording()`, symmetrischer Gegenpart zu `upgradeToPlayAndRecord`, Session bleibt aktiv auf `.playback`). Entriegelt **#13**. **Reviewer-Präzisierungen (wichtig, keine Blocker):** (a) **PRÄVENTIVE Härtung, kein aktiv feuernder Bug** — der Mic-Record-Pfad ist heute DORMANT (`AudioEngine.inputMonitoringEnabled` wird nirgends `true` gesetzt); der alte `setActive(false)` war zudem redundant mit dem `stop()`-Teardown. Kein Deploy-Hörtest (nichts user-sichtbar). (b) `recordingRouteNeeded` ist EIN geteiltes, NICHT-refcountetes Bool über 3 Konsumenten (MicrophoneManager, `AudioEngine.setInputMonitoring`, `MultiTrackRecorder`) — heute safe (stopRecording läuft nur aus `AudioEngine.stop()`, das alles abreißt), aber vor einem Entkoppeln des Mic-Pfads zu einem Refcount/`any-consumer-active` härten. (c) **Der wahrscheinlichere LIVE-Stille-Pfad (falls je wieder reproduziert):** `AudioEngine.stop():632` eigener `setActive(false)` + Route-Change-Handling — NICHT von diesem Commit berührt; #22 gilt als gefixt (v198), kein aktueller Report → nicht blind hinterherjagen.
