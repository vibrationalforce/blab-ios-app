...
- **MIDI is input here.** CoreMIDI notes, per-channel pitch bend, CC 74 and CC 21–31 (air controllers) arrive as `ControllerEvent`. Full MPE (master/member disambiguation, RPN 6,6 zones, channel pressure) is *not* wired — see the header of `Sync/MIDIBusPublisher.swift`. This bullet is about the BUS: MIDI note OUT and MIDI file export exist and are listed under the output stage below. Outbound expression (pitch bend, CC 74, channel pressure) is gated behind `mpeEnabled`/`expressionEnabled`, and neither flag has a writer in `Sources/` — so live MIDI out is note-only, MIDI 1.0.
