...
| O5 | Per-Instrument EchoelSynth (#23) | ältere Serie | **PLAN+Council ✓** (`PLAN_PER_LANE_PATCH.md`). **S1 ✓ SHIPPED (b2f7750):** `TimelineStore.setLanePatch(_:patch:)` + 4 Tests [pur, code-reviewer 0 Defekte, Gates pending]. Slices offen: **S2 .patch(lane)-Payload** (nächste, reviewer-gated) → S2b Primär-Sink → S3 Live-Apply [gerät-gated]. | Patch-Wechsel pro Spur hörbar (2 Spuren, 2 Klänge) |
