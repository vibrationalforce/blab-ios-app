...
> - **EchoelVis is LIVE** — `Views/MetalBioView.swift` is the live full-screen bio visual (HR→pulse ≤2.5 Hz WCAG, coherence→hue, breath→spread, honours Reduce Motion). NOT dormant/deprecated. **EchoelSeq = 19 offered genres** (corrected 2026-08-06 to 16, 2026-09-04 to 19 with #983 S3–S5 `deepTech`/`darkMinimal`/`psyProgHouse`; #254 added eight and every claim of "8" went stale — the "23"/"12" figures before that were both wrong too. Count with `MusicStyle.offered.count`, never from memory). **sACN unicast is LIVE** beside Art-Net.
