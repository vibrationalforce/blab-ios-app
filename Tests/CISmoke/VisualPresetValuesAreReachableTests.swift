...
final class VisualPresetValuesAreReachableTests: XCTestCase {
    ...
    func testEveryVisualRowSaysHowManyDecimalsItShows() throws {
        ...
        XCTAssertEqual(calls.count, 10, """
            `visualAdjustFields` no longer holds ten `EchoelValueField` rows — it holds \
    ...
    ///
    /// Paren-matched rather than line-based on purpose: four of the ten rows wrap, and each of
