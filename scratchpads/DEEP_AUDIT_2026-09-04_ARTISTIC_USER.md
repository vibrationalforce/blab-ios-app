...
**Slice:** delete the stale door assertion at `AStillIsOneFrameNotASecondPathTests.swift:86-89`. Do **not** re-point it at `StillShutterButton(recorder:` — that needle already exists verbatim at `TheStillSaysWhetherItWasSavedTests.swift:97` and duplicating it is the #416 trap. ⛔ CORRECTED 2026-09-07 (#1050): this said "Keep the `.sheet(` == 14 non-growth check at `:92`; it still measures correctly." It did NOT. That guard read the file UNSTRIPPED, so its 14 was 9 real `.sheet(` call sites plus 5 mentions inside comments — it drifted on prose edits and was blind to a new `.fullScreenCover`. It is retracted whole; the claim's one proper home is `ResetSoundClearsWhatTheLaunchLineReportsTests.testTheConfirmationDidNotBecomeAnotherModal` (comment-stripped, six modifier forms, file-wide `== 16` AND chain `<= 14`).
