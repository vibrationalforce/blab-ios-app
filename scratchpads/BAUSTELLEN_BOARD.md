...
| A5 | **BodyVibe B2 — Kamera-Modulator Stufe 1** | „Grimassen/Smile/Arme als Modulator" (07-17) | FUNDAMENT ✓ (d624fcc, Gates laufen): `FaceExpressionMapping` pure-Core (rate-EMA, quellen-agnostisch) + BioSampleFrame +3 additive Felder + ModSource `.faceSmile/.faceBrow/.faceJaw`. KEIN Deploy (nichts geräte-sichtbar). Nächste: ARKit-Front-Face-Publisher (Bio/*) + `FeatureFlags.cameraExpression` OFF + Info.plist-Permission + `applyRouting`-Wiring (wie `blehrs.in`) + rPPG-Kamera-Konflikt | Lächeln verändert hörbar die Komposition — **Gerät offen (folgt mit Publisher-Slice)** | RESEARCH_BODYVIBE_CAMERA; Copy: „Expression/Steuerung", NIE „Emotion" |
