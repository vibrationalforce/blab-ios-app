...
struct EchoelStudioView: View {
    ...
    private var soundControls: some View {
        VStack(alignment: .leading, spacing: 12) {
            // Each panel is AnyView-erased. `soundControls` is rendered in the main
            // scroll at launch, and its concrete type is the SUM of all panel sub-types.
            // Inlined, that TupleView grew (visualPanel gained the Bio→Visual section in
            // 10.76.10) until the Swift runtime's type-metadata decoder overflowed the
            // main-thread stack on first build → SIGSEGV / black screen at launch (the
            // 10.76.10+ launch brick; 10.76.9 was just under the limit). AnyView puts a
            // concrete boundary at each panel so the decoder never sums their depth, and
            // each panel is decoded independently with plenty of headroom. Cheap — these
            // are page-level containers, not tight-loop rows. DO NOT inline these again.
            AnyView(compositionPanel)
            AnyView(transposePanel)
            AnyView(moodPanel)
            AnyView(soundPanel)
            AnyView(effectsPanel)
            AnyView(masterPanel)
            AnyView(visualPanel)
            if running {
