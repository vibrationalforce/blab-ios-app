...
// only control when the Tools grid was removed (2026-07-02), so from that day a player routing
// to a hardware rig got plain channel-1 notes and had no way to ask for anything else. CLAUDE.md
// has carried them as an open capability gap ever since ("gehören in die Routing-Fläche").
//
...
final class MIDIOutQualitySwitchesTests: XCTestCase {
    ...
    func testNothingOutsideTheEngineAssignsTheFlags() throws {
        let offenders = try swiftFilesUnderSources()
            ...
            .filter { file in
                SourceText.codeOnly(file.value)
                    .split(separator: "\n", omittingEmptySubsequences: false)
                    .contains { line in
                        (line.contains("mpeEnabled =") || line.contains("expressionEnabled ="))
                            && !line.contains("==")
                    }
            }
            .map(\.key)
    ...
    /// `applyOutputPreferences()` is useless without callers, and it needs BOTH: the launch path
    /// so a stored preference survives a relaunch, and the switches so a change takes effect now.
    func testTheOwnerIsCalledAtLaunchAndFromTheSwitches() throws {
        let engine = try codeOf(Self.engine)
            .split(separator: "\n", omittingEmptySubsequences: false)
            .map(String.init)
        ...
        let surface = try codeOf(Self.surface)
        XCTAssertEqual(surface.components(separatedBy: "midiOut.applyOutputPreferences()").count - 1, 2, """
            Routing no longer applies BOTH switches through the owner. Each toggle needs its own \
            `.onChange` calling it — otherwise one switch moves the stored key and the live engine \
            keeps the old value until the next launch.
            """)
    }
