...
final class SACNSenderTests: XCTestCase {
    ...
    func testPacket_totalLengthIsFullUniverse() {
        let p = SACNSender.e131Packet(universe: 1, sequence: 0, cid: cid, channels: [1, 2, 3])
        // 126-byte header + 512 slots = 638.
        XCTAssertEqual(p.count, 638)
    }

    func testPacket_rootLayerIdentifiers() {
        let p = [UInt8](SACNSender.e131Packet(universe: 1, sequence: 0, cid: cid, channels: []))
    ...
    func testPacket_framingVectorPriorityUniverseAndSequence() {
        let p = [UInt8](SACNSender.e131Packet(universe: 0x0102, sequence: 7, cid: cid, channels: []))
    ...
    func testPacket_dmpStartCodeAndPropertyCount() {
        let p = [UInt8](SACNSender.e131Packet(universe: 1, sequence: 0, cid: cid, channels: [9, 8, 7]))
    ...
    func testPacket_channelsPaddedAndClampedTo512() {
        ...
        let p = SACNSender.e131Packet(universe: 1, sequence: 0, cid: cid, channels: over)
