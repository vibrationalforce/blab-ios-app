...
> - **EchoelVis is LIVE** — `Views/MetalBioView.swift` is the live full-screen bio visual (HR→pulse ≤2.5 Hz WCAG, coherence→hue, breath→spread, honours Reduce Motion). NOT dormant/deprecated. **EchoelSeq = 17 offered genres** (corrected 2026-08-06 to 16, 2026-09-04 to 17 with #983 S3 `deepTech`; #254 added eight and every claim of "8" went stale — the "23"/"12" figures before that were both wrong too. Count with `MusicStyle.offered.count`, never from memory). **sACN unicast is LIVE** beside Art-Net.
...
- **Character params — LIVE (2026-06-17):** `MoodProfile` now 8 dims (… + **virtuosity, syncopation, humanize**), all consumed in the lead generator. **17 offered genres** (`MusicStyle.offered`; the enum holds 34 cases, so **17** are not offered — the curation is by ear and re-offering one is a listening decision. ⛔ This said "nine older ones": 34 − 17 = 17 (33 − 16 before #983 S3 added `deepTech` to both sides), and it was 17 at the 2026-07-24 curation too (25 declared, 8 offered). Nothing in the tree counts to nine).
