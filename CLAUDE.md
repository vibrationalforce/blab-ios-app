...
⛔ **Drei Wörter sind hier am 2026-07-31 gestrichen worden, weil sie keinen Produzenten haben** — und diese Zeile ist die erste, die eine Session liest: **„motion"** (`ModulationMatrix.hasProducer` gibt für `.motion` hart `false` zurück; jede `BioSampleFrame`-Konstruktion in `Sources/` schreibt `motionEnergy: 0`, der letzte CoreMotion-Provider ging im 2026-06-19-Cleanup) · **„brain rhythm"** (`.eegBurst` hat in ganz `Sources/` genau drei Vorkommen: den Enum-Case, eine OSC-Adresszuordnung und einen Consumer-`switch` — NULL Produzenten) · **„broadcast"** (`Package.swift` `dependencies: []`, HaishinKit nicht verlinkt; RTMP kann als Codepfad nicht existieren). Sie standen hier, während dieselbe Datei 200 Zeilen tiefer jeden einzelnen als abwesend führte.
...
| Layer | Framework |
|---|---|
| Apple (iPhone) | AVFoundation + Accelerate + Metal + CoreMIDI (+ HealthKit · CoreBluetooth · CoreLocation · WeatherKit · MultipeerConnectivity, je hinter `canImport`) |
...
Tests/EchoelmusicTests/ ← 314 test files (`git ls-files 'Tests/EchoelmusicTests/*.swift' | wc -l`,
                          2026-07-29). ⛔ Drei Zahlen kursierten für DIESE eine Suite: 305 hier,
                          294 in den Schrittnamen von `full-tests.yml`, 311 auf der Platte am
                          2026-07-28 — heute 314. Die Workflow-Beschriftung ist founder-gated und
                          bleibt vorerst falsch (#208).
                          Und die Suite ist NICHT das blockierende Bundle — das baut aus
                          `Tests/CISmoke` (**58** Dateien, `git ls-files 'Tests/CISmoke/*.swift' | wc -l`,
                          2026-07-31 nach dem Dateinamen-ASCII-Wächter; am selben Tag früher „57", am 2026-07-30 „56", „55", „54", „53", „52", „51", „50", „49", „48", „47", „46", „45", „41", „39", am
...
- **`Echoelmusic CI/CD Pipeline`** macht `build-for-testing` (`ci.yml:175`) **und** `test-without-building` (`ci.yml:190`), beide mit `set -o pipefail` und ohne `continue-on-error` (ein Kommentar bei `ci.yml:198-202` verbietet die frühere `|| cat`-Maske ausdrücklich). **Es ist das EINZIGE Gate, das `Tests/CISmoke` kompiliert UND ausführt.**
