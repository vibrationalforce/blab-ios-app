...
- **Root view:** `Studio/WorkspaceView.swift` (`@main` → `WindowGroup` → `WorkspaceView`) — the workspace shell. **DAW-look reorg (2026-07-01, v10.77.5–v10.78.0):** bottom-bar surfaces are now **Arrange · Clips · Compose · Mix · Bio · Browse** (6 tabs, all MOUNTED via `surfaceLayer` ZStack+opacity — switching never unmounts Compose's audio lifecycle). A **persistent transport bar** (`TransportBar` + `TransportPositionView` leaves) sits in the chrome under the brand header on every surface: Play/Stop drives `PatternEngine`, the Tempo field is `EchoelValueField` (compact `boxWidth`), and the ~10 Hz bars.beats position lives in its own leaf (freeze rule). **Mix** = `ChannelRackView(embedded:true)`; **Bio** = `BioSourceView` (biofeedback repositioned as ONE modulation SOURCE — arm the body, live metrics, route Body→sound/visual, entrainment+safety; shares the one `cameraRPPG` with Compose, `isRunning` is truth); **Browse** = `BrowserView` (recall synth presets + audition samples). `Studio/EchoelStudioView.swift` is the **Compose surface** (still with its own `BioStripView` at top), NOT the app root. Add a surface via the `Surface` enum + `surfaceLayer` (safe additive pattern — never a root rewrite). **Video page = designed + DEFERRED** (needs device verification; `scratchpads/PLAN_VIDEO_PAGE.md`). The old `StudioRoot` Tools/Works/Sync/Well TabView is removed.
...
- **Absent (not wired — do not claim as shipping):** RTMP/streaming (BroadcastPublisher is a compile-safe scaffold behind `#if canImport(HaishinKit)`; HaishinKit not integrated), video capture/edit, multitrack audio. **Art-Net + sACN (unicast) are live.** **VocoderCore / BioModulation** = pure tested cores, **not yet wired** (foundations). **FeedbackGuard** (audio-input live monitoring) and **BioVisualParams** (immersive flash-safe pulse) are **wired**. See `docs/dev/FEATURE_MATRIX.md`.
